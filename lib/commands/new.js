 ...
 * @private
 */
function _prompts(projName, stage, s3Bucket, notificationEmail, region, awsProfile) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    prompts.properties.s3Bucket = {
      ...
      default: (data.projName ? data.projName : projectName) + '.myapp.com',
    ...
    prompt.get(prompts, function(err, result) {
      ...
      .then(function(answers) {
        ...
        if (data.region) return Promise.resolve(data);

        // Show region Select
        var choices = [
          { key: '1) ', value: 'us-east-1' },
          { key: '2) ', value: 'us-west-1' },
          { key: '3) ', value: 'eu-west-1' },
          { key: '4) ', value: 'ap-northeast-1' },
        ];

        return JawsCLI.select('Select a region for your project: ', choices, false)
            .then(function(results) {
              data.region = results[0].value;
              return Promise.resolve(data);
            });
      })
      .then(function(answers) {

        if (data.awsProfile) return Promise.resolve(data);

        // Show profiles checklist
        ...
        return JawsCLI.select('Select a profile for your project: ', choices, false)
            .then(function(results) {
              data.awsProfile = results[0].value;
              return Promise.resolve(data);
            });
      });
}
        ...
        return new Promise(function(resolve, reject) {
          ...
          async.whilst(
              ...
              function() {
                // Stop Spinner, inform
                spinner.stop(true);
                JawsCLI.log('CloudFormation Stack ' + stackData.Stacks[0].StackName + ' successfully created.');
 ...
 * @returns {*}
 */
module.exports.new = function(projName, stage, s3Bucket, notificationEmail, region, awsProfile, noExeCf) {

  // ASCII Greeting
  JawsCLI.ascii();

  return _prompts(projName, stage, s3Bucket, notificationEmail, region, awsProfile)
